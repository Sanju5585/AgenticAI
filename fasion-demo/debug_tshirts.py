...
def debug_tshirt_accessories():
    ...
    try:
        ...
        for tshirt_name in tshirt_products:
            ...
            cursor.execute("SELECT PRODUCT_ID, PRODUCT_NAME FROM SAP_PRODUCTS_COMMERCE_2211_V2 WHERE UPPER(PRODUCT_NAME) LIKE UPPER(?)", (f'%{tshirt_name}%',))
                ...
                for result in results:
                    ...
                    if acc_result and acc_result[0]:
                        ...
                        for acc_id in accessory_ids[:3]:  # Show first 3
                            cursor.execute("SELECT PRODUCT_NAME, PRICE FROM SAP_PRODUCTS_COMMERCE_2211_V2 WHERE PRODUCT_ID = ?", (acc_id,))
        ...
        for pattern in patterns:
            cursor.execute("SELECT PRODUCT_ID, PRODUCT_NAME FROM SAP_PRODUCTS_COMMERCE_2211_V2 WHERE UPPER(PRODUCT_NAME) LIKE UPPER(?) LIMIT 5", (pattern,))
            results = cursor.fetchall()
        ...
        for product_id in known_tshirt_ids:
            cursor.execute("SELECT PRODUCT_NAME FROM SAP_PRODUCTS_COMMERCE_2211_V2 WHERE PRODUCT_ID = ?", (product_id,))
